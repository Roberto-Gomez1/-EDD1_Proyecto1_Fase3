...
let cadenagrafo = ''
let cnt = 0
let grafolista = JSON.parse(localStorage.getItem("grafoBloc")) || []
...
function reporte() {
  ...
  bloque_actual = bloque.inicio
  if (bloque_actual != null) {
    cadenagrafo += `n${cnt}[label="TimeStamp=${bloque_actual.valor['timestamp']}  Emisor: ${bloque_actual.valor['transmitter']}  Receptor: ${bloque_actual.valor['receiver']}  PreviousHash: ${bloque_actual.valor['previoushash']}"]}`;
